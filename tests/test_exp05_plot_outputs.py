"""Tests for Experiment 5 plotting from existing artifacts."""
...
def plot_module():
    import experiments.plot_exp05_figures as module
...
def test_require_columns_accepts_present_columns(plot_module):
    df = pd.DataFrame({"a": [1], "b": [2]})

    plot_module.require_columns(df, ["a", "b"], "dummy.csv")


def test_require_columns_reports_missing_columns(plot_module):
    df = pd.DataFrame({"a": [1]})
...
def test_find_grid_best_feasible_uses_largest_allowed_c(plot_module):
    grid = pd.DataFrame(
        {
            "curtailment_factor": [0.0, 0.5, 0.7, 0.72, 1.0],
            "p_export_mw": [5.4, 6.5, 6.95, 7.02, 7.6],
        }
    )

    best = plot_module.find_grid_best_feasible(grid, limit_mw=7.0)

    assert best["curtailment_factor"] == pytest.approx(0.7)
    assert best["p_export_mw"] == pytest.approx(6.95)


def _write_dummy_artifacts(exp05a_dir: Path, exp05b_dir: Path) -> None:
    exp05a_dir.mkdir(parents=True, exist_ok=True)
    exp05b_dir.mkdir(parents=True, exist_ok=True)
    ...
    baseline.to_csv(exp05b_dir / "selected_case_baseline.csv", index=False)
    final.to_csv(exp05b_dir / "final_solution.csv", index=False)
    grid.to_csv(exp05b_dir / "grid_reference.csv", index=False)
    trace.to_csv(exp05b_dir / "optimization_trace.csv", index=False)


def test_generate_figures_from_dummy_artifacts(plot_module, tmp_path: Path):
    exp05a_dir = tmp_path / "exp05a"
    exp05b_dir = tmp_path / "exp05b"
    output_dir = tmp_path / "figures"
    _write_dummy_artifacts(exp05a_dir, exp05b_dir)

    generated = plot_module.generate_figures(
        exp05a_dir=exp05a_dir,
        exp05b_dir=exp05b_dir,
